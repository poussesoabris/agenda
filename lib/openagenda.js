 ...
 *  - API v2 (officielle, pérenne) si la clé est fournie via la variable
 *    d'environnement OPENAGENDA_KEY (secret GitHub Actions en CI) ;
 *  - export legacy public (déprécié par OpenAgenda) en repli, tant que
 *    l'asso n'a pas transmis de clé.
 * Les deux chemins sont normalisés vers le même format d'événement interne :
 * la suite (buildOccurrences, tests, contrat) ne voit pas la différence.
 */

function sourceUrl(uid = AGENDA_UID) {
  return process.env.OPENAGENDA_KEY
...
async function fetchAllEvents(uid = AGENDA_UID) {
  return process.env.OPENAGENDA_KEY
    ? fetchAllEventsV2(uid, process.env.OPENAGENDA_KEY)
    : fetchAllEventsLegacy(uid);
}

async function fetchAllEventsLegacy(uid) {
  const src = `https://openagenda.com/agendas/${uid}/events.json`;
...
async function fetchAllEventsV2(uid, key) {
  ...
  for (let guard = 0; guard < 50; guard++) {
    ...
    if (!r.ok) throw new Error(`OpenAgenda API v2 HTTP ${r.status}`);
    const d = await r.json();
...
function normalizeV2Event(e, uid) {
  ...
  return {
    ...
    timings: (e.timings || []).map((t) => ({ start: t.begin || t.start, end: t.end })),
