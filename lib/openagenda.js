...
async function fetchAllEventsV2(uid, key) {
  ...
  for (let guard = 0; guard < 50; guard++) {
    ...
    events = events.concat((d.events || []).filter(isPubliable).map((e) => normalizeV2Event(e, uid)));
...
/**
 * Ne publie que les événements réellement publics : l'API v2 renvoie aussi les
 * brouillons, les événements privés et ceux encore en modération, qui n'ont
 * rien à faire dans un flux destiné au site public.
 * `state` : 0 = en modération, 1 = prêt à publier, 2 = publié (seul retenu).
 */
function isPubliable(e) {
  return e.state === 2 && !e.draft && !e.private && e.valid !== false;
}
...
function normalizeV2Event(e, uid) {
  ...
  return {
    ...
    // Code de statut OpenAgenda (annulé, complet, reporté…), transmis tel quel :
    // l'énumération n'est pas documentée publiquement et seule la valeur 1
    // (déroulement normal) a été observée. Voir docs/CONTRAT-API.md.
